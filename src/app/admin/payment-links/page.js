...
export default function PaymentLinksPage() {
  ...
  function updateDropdownOption(fieldIndex, optIndex, updates) {
    const field = customFields[fieldIndex];
    updateCustomField(fieldIndex, {
      options: field.options.map((o, i) => (i === optIndex ? { ...o, ...updates } : o)),
    });
  }
  ...
  async function handleCreate(e) {
    ...
    if (priceSource === "existing") {
      ...
      if (!inlineName || !inlineAmount) {
        setError("Name and amount are required for a new product");
        setCreating(false);
        return;
      }
      body.inlinePrice = {
    ...
    if (adjustableEnabled) {
      body.adjustableQuantity = {
        enabled: true,
        minimum: adjustableMin,
        maximum: adjustableMax,
      };
    }
    ...
    <div>
      ...
      {success && (
        <div className="bg-green-50 text-green-700 text-sm px-4 py-3 rounded-lg border border-green-200 mb-6">
          ...
          <a href={success} target="_blank" rel="noopener noreferrer" className="font-medium underline break-all">
            {success}
          </a>
        </div>
      )}
      ...
      {view === "create" ? (
        <CreateForm
          ...
          pricesFlat={pricesFlat}
          inlineName={inlineName}
          setInlineName={setInlineName}
          inlineAmount={inlineAmount}
          setInlineAmount={setInlineAmount}
          inlineCurrency={inlineCurrency}
          setInlineCurrency={setInlineCurrency}
          inlineRecurring={inlineRecurring}
          setInlineRecurring={setInlineRecurring}
          inlineInterval={inlineInterval}
          setInlineInterval={setInlineInterval}
          quantity={quantity}
          setQuantity={setQuantity}
          adjustableEnabled={adjustableEnabled}
          setAdjustableEnabled={setAdjustableEnabled}
          adjustableMin={adjustableMin}
          setAdjustableMin={setAdjustableMin}
          adjustableMax={adjustableMax}
          setAdjustableMax={setAdjustableMax}
          submitType={submitType}
          setSubmitType={setSubmitType}
          isRecurring={isRecurring}
          allowPromotionCodes={allowPromotionCodes}
          setAllowPromotionCodes={setAllowPromotionCodes}
    ...
    <form onSubmit={handleCreate} className="space-y-6 max-w-2xl">
      ...
      <Section title="Product">
        ...
        {priceSource === "existing" ? (
          <select
            ...
            className="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
          >
            <option value="">Select a product...</option>
            {pricesFlat.map((p) => (
              <option key={p.priceId} value={p.priceId}>
                {p.productName} — ${(p.amount / 100).toFixed(2)} {p.currency.toUpperCase()}
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-3">
              ...
              <select
                ...
                className="px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
              >
            ...
            {inlineRecurring && (
              <select
                value={inlineInterval}
                onChange={(e) => setInlineInterval(e.target.value)}
                className="px-4 py-2.5 border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
              >
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-20 px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
            />
          </div>
          <label className="flex items-center gap-2 text-sm text-gray-700">
          ...
          <div className="flex items-center gap-4 mt-3">
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                min="1"
                value={adjustableMin}
                onChange={(e) => setAdjustableMin(parseInt(e.target.value) || 1)}
                className="w-20 px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
              />
            </div>
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                min="1"
                value={adjustableMax}
                onChange={(e) => setAdjustableMax(parseInt(e.target.value) || 10)}
                className="w-20 px-3 py-2 border border-gray-300 rounded-lg text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
              />
            </div>
          </div>
      ...
      <Section title="Custom Fields">
        {customFields.length === 0 && (
          <p className="text-sm text-gray-400 mb-3">No custom fields added yet</p>
        )}
        ...
        <button
          ...
          className="mt-3 px-4 py-2 text-sm font-medium text-blue-600 border border-blue-300 rounded-lg hover:bg-blue-50 transition"
        >
          + Add Custom Field
        </button>
      </Section>
  ...
  return (
    <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
      ...
      <div className="grid grid-cols-2 gap-3 mb-3">
        <div>
          <label className="block text-xs text-gray-500 mb-1">Key (unique identifier)</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-xs text-gray-500 mb-1">Label (shown to customer)</label>
          <input
            type="text"
        ...
        <div className="flex items-end pb-1">
          <label className="flex items-center gap-2 text-sm text-gray-700">
            <input
              ...
              onChange={(e) => updateCustomField(index, { optional: e.target.checked })}
            ...
            {field.options.map((opt, oi) => (
              <div key={oi} className="flex gap-2 items-center">
                <input
                  ...
                  placeholder="Label"
                  value={opt.label}
                  onChange={(e) => updateDropdownOption(index, oi, { label: e.target.value })}
                  className="flex-1 px-3 py-1.5 border border-gray-300 rounded text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                />
                <input
                  type="text"
                  placeholder="Value"
                  value={opt.value}
                  onChange={(e) => updateDropdownOption(index, oi, { value: e.target.value })}
                  className="flex-1 px-3 py-1.5 border border-gray-300 rounded text-sm text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"
                />
                {field.options.length > 1 && (
                  <button type="button" onClick={() => removeDropdownOption(index, oi)} className="text-red-400 text-sm hover:text-red-600">
                    &times;
                  </button>
                )}
              </div>
            ...
            <tr key={link.id} className="hover:bg-gray-50 transition">
              ...
              <td className="px-6 py-4">
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                  link.active ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"
                }`}>
