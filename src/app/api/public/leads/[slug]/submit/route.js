...
export async function POST(request, { params }) {
  try {
    ...
    if (lead.status !== "enabled") {
      return NextResponse.json({ error: "Lead is not accepting submissions" }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, submissionId: submission._id }, { status: 201 });
  } catch (error) {
