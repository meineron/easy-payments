...
export async function POST(request, { params }) {
  try {
    ...
    if (!numPayments || numPayments < 1 || numPayments > 12) {
      return NextResponse.json({ error: "Number of payments must be between 1 and 12" }, { status: 400 });
    }
    if (!parentFirstName || !parentLastName || !parentEmail || !parentPhone) {
      return NextResponse.json({ error: "Parent first name, last name, email, and phone are required" }, { status: 400 });
    }
