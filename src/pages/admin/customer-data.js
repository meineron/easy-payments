...
export default function CustomerDataPage() {
  ...
  function cardBrandLabel(brand) {
    const brands = { visa: "Visa", mastercard: "Mastercard", amex: "Amex", discover: "Discover", diners: "Diners", jcb: "JCB", unionpay: "UnionPay" };
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-gray-900 mb-6">Customer Stripe Data</h2>
      ...
      <tr className="hover:bg-gray-50 transition cursor-pointer" onClick={onToggle}>
          ...
          {tx.refunded ? (
            <div>
              <span className="text-red-600 font-medium">
                {formatAmount(tx.amountRefunded, tx.currency)}
              </span>
        ...
        <td className="px-4 py-4 text-sm whitespace-nowrap">
          <div className="flex gap-2">
            {tx.receiptUrl && (
              <a href={tx.receiptUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline" onClick={(e) => e.stopPropagation()}>
                Receipt
              </a>
            )}
            {tx.invoiceUrl && (
              <a href={tx.invoiceUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline" onClick={(e) => e.stopPropagation()}>
                Invoice
              </a>
            )}
            {tx.invoicePdf && (
              <a href={tx.invoicePdf} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline" onClick={(e) => e.stopPropagation()}>
                PDF
              </a>
            )}
          </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <dl className="space-y-2 text-sm">
                  ...
                  {tx.card && (
                    <div className="flex gap-2">
                      <dt className="text-gray-500 min-w-[140px]">Card:</dt>
                      <dd className="text-gray-900">
                        {tx.card.brand?.toUpperCase()} **** {tx.card.last4} — Exp {String(tx.card.expMonth).padStart(2, "0")}/{tx.card.expYear}
                      </dd>
                    </div>
                  )}
                </dl>
                ...
                <td className="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">
                  {price ? (
                    <>
                      ...
                      {isRecurring && (
                        <span className="text-gray-400 font-normal"> / {price.recurring.interval}</span>
                      )}
                    </>
                  ) : "—"}
                </td>
