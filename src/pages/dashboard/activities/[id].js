    ...
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          ...
          {activity?.season && (
            <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-gray-600" title={t("season")}>
              {activity.season}
            </span>
          )}
        </div>
        <Link href={`/dashboard/activities/${activityId}/edit`}>
          <a className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-700 text-center w-full sm:w-auto">
            {td("editActivity")}
          </a>
        </Link>
      </div>
...
ActivityPage.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;
