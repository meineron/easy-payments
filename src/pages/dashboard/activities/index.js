...
export default function ActivitiesPage() {
  ...
  async function toggleStatus(id, currentStatus) {
    ...
    try {
      ...
      setActivities((prev) => prev.map((a) => (a._id === id ? { ...a, status: newStatus } : a)));
    } catch {
              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                ...
                >
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${a.status === "published" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    ...
                    className={`text-xs px-3 py-1 rounded font-medium ${a.status === "published" ? "bg-yellow-50 text-yellow-700 hover:bg-yellow-100" : "bg-green-50 text-green-700 hover:bg-green-100"}`}
                  >
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setShowCreateModal(false)}>
          <div className="bg-white rounded-xl border border-gray-200 p-6 w-full max-w-md shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowCreateModal(false)} className="flex-1 px-4 py-2.5 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition">
                {tc("cancel")}
              </button>
              <button
...
ActivitiesPage.getLayout = (page) => <DashboardLayout>{page}</DashboardLayout>;
